# Capability: Dynamic Model List

## Purpose

由後端動態從 Groq API 取得最新可用的模型清單並回傳給前端，避免硬編碼的模型清單在 Groq 上線新模型或下線舊模型時產生不一致。後端使用記憶體快取降低 API 呼叫成本，並在 Groq API 暫時不可用時提供靜態 fallback 清單。
